...
export interface Exercise {id:string;name:string;muscle_group:MuscleGroup;load_type:LoadType;notes:string;created_at?:string}
export interface RoutineExercise {id:string;order_index:number;default_weight:number;default_band_name:string;exercise:Exercise}
export interface Routine {id:string;name:string;muscle_group:MuscleGroup;routine_exercises:RoutineExercise[]}
export interface WorkoutSet {id:string;order_index:number;repetitions:number;weight:number;band_name:string;is_completed:boolean}
export interface WorkoutExercise {id:string;name:string;muscle_group:MuscleGroup;load_type:LoadType;order_index:number;note:string;workout_sets:WorkoutSet[]}
